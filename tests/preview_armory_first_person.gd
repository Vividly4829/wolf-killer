...
func run() -> void:
 ...
 for pose in ["hip","aim","reload"]:
  for page in 3:
   var sheet:=Image.create(1920,810,false,Image.FORMAT_RGBA8)
   for cell in 12:
    var id:=page*12+cell; game.select_weapon(id)
    ...
    sheet.blit_rect(picture,Rect2i(0,0,480,270),Vector2i(cell%4*480,cell/4*270))
   sheet.save_png("res://qa/armory-fps-%s-%d.png"%[pose,page])
 print("FIRST_PERSON_REVIEW_COMPLETE: 36 weapons / hip, aim, reload")
 quit()
