...
func _pause() -> void:
	...
	_label(overlay, "Your teammates keep playing." if is_instance_valid(game.split_session) else ("Online world keeps running." if game.coop.active else "The island can wait."), Vector2(470, 237), 17, MUTED)
...
func _draw_map() -> void:
	...
	draw_string(body_font, Vector2(1067, 220), "SKULLS: dangerous wildlife / people" if game.rituals.all_radar() else "75 m: RED hostile / BLUE panic", HORIZONTAL_ALIGNMENT_LEFT, 182, 10, Color("d4e0e5"))
