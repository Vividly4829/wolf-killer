...
func _ready() -> void:
	...
	max_health=110 if species in ["raider","legionary"] else 260
	...
	body.radius=.30 if species in ["raider","legionary"] else .52; body.height=1.8 if species in ["raider","legionary"] else 2.0
	shape.shape=body; shape.position.y=.90
	if species=="bear": shape.rotation.x=PI/2
	hit.add_child(shape)
	var organs: Array=preload("res://scripts/human_xray.gd").ORGANS if species in ["raider","legionary"] else preload("res://scripts/wildlife_anatomy.gd").organs("bear")
	for organ in organs:
...
func visible_to(p: Vector3) -> bool:
	var ray=PhysicsRayQueryParameters3D.create(position+Vector3.UP*(1.5 if species in ["raider","legionary"] else .9),p+Vector3.UP,1)
...
func hear(origin: Vector3) -> void:
	...
	if not alerted:
		...
		if position.distance_to(game.player.position)<45: game.show_notice("A bear huffs and stamps." if species=="bear" else "Shields raised! Legionaries advance." if species=="legionary" else "A raider shouts: Who's there?",3)
...
func choose_target() -> Node3D:
	...
	if species in ["raider","legionary"]:
		for enemy in game.wolves+game.nodes_in_group("campaign_threats"):
			if enemy==self or enemy.dead or enemy.get("species") in ["raider","legionary"]: continue
		...
		for enemy in game.nodes_in_group("campaign_threats"):
			if enemy.species in ["raider","legionary"] and not enemy.dead and position.distance_to(enemy.position)<minf(best,18): result=enemy; best=position.distance_to(enemy.position)
	return result
func _physics_process(delta: float) -> void:
	if dead or is_queued_for_deletion() or game.coop.client() or not game.is_playing(): return
	if species in ["raider","legionary"]: model.get_child(0).set_motion(2.8 if not route.is_empty() else 0,false,alerted,species=="legionary" and cooldown>.85)
	...
	if think<=0:
		...
		if species=="legionary": goal=formation_goal(goal)
		if not search:
	...
	if is_instance_valid(target) and warning<=0 and cooldown<=0 and visible_to(target.position):
		...
		if species=="raider" and range_to<42: shoot()
	...
	if is_instance_valid(target) and species in ["raider","legionary"]:
...
func damage(amount: float,reward_hunter: bool=false) -> void:
	...
	if health<=0:
		...
		if species in ["raider","legionary"]: model.get_child(0).set_process(false)
